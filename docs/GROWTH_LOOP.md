...
| Analytics | PostHog project "Skin Considered" (id 589734, US cloud) | Page views, page leaves, and `newsletter_submit` events with `source` and `outcome` (preview, success, error). Read through the PostHog connector attached to the routine (or the PostHog MCP locally). The site sends events only when `NEXT_PUBLIC_POSTHOG_KEY` is set, which it is in Vercel production and preview. Agents may create insights, dashboards, and actions in this project; they must not enable session recording, autocapture, surveys, or any capture of form contents |
