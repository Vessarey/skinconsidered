...
| Analytics | PostHog project "Skin Considered" (id 589734, US cloud); dashboard "Skin Considered growth" (id 2056720) | Events: `$pageview`, `$pageleave` (scroll depth), `$web_vitals`, `newsletter_view {source}`, `newsletter_submit {source, outcome}`, `outbound_click {domain, path}`, `related_click {to_kind, path}`, `cta_click {label, path}`, `procedure_filter {group, value}`, `procedure_expand {slug}`, `site_search {results, zero}`. Read through the PostHog connector attached to the routine (or the PostHog MCP locally). The site sends events only when `NEXT_PUBLIC_POSTHOG_KEY` is set, which it is in Vercel production and preview. Agents may add insights to the dashboard and define actions; they must not enable session recording, autocapture, surveys, heatmaps, or any capture of form contents or search text, and any new event must be named on /privacy in the same PR |
| Search | Google Search Console property `sc-domain:skinconsidered.com` | `npm run gsc -- sites\|queries\|pages\|flywheel\|dates\|entry [days]` and `npm run gsc -- inspect <url>`. Credential: `GSC_SERVICE_ACCOUNT_JSON_B64` in the environment (or `docs-internal/gsc-service-account.json` locally, gitignored). `flywheel` lists queries ranking 8–20, the highest-leverage content targets; `entry` lists high-impression pages with CTR for title and description work; `dates` is the honest read on whether a change moved search traffic. Data lags about two days and starts empty for a new property |
| Secondary analytics | Vercel Web Analytics | Cookieless page views and referrers as a cross-check; readable via the Vercel API only if `VERCEL_TOKEN` is set in the routine's environment |
...
3. **Data.** Run `npm run gsc -- dates 28`, `npm run gsc -- entry 28`, and `npm run gsc -- flywheel 28` for search demand and click-through; judge whether a title or description change worked on the `dates` trend, not on referrer counts. Then, using the PostHog connector against project 589734, pull the last 7 and 28 days: `$pageview` counts by path and referrer, entry pages, `newsletter_submit` by `source` and `outcome`, and pages with high views but no onward click. Keep one dashboard named "Skin Considered growth" current with these insights; create it on the first run if it does not exist. Turn findings into concrete edits: stronger titles and descriptions on high-impression pages, internal links from popular files to related ones, a newsletter placement on a page that gets traffic but no signups, a new file for a topic readers search for internally. Record each decision and the number behind it in the PR description. No analytics means no data claims.
